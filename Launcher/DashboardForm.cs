...
internal sealed class RaceControlPanel : UserControl
{
    ...
    private readonly DataGridView _tyreInventory = new();
    ...
    private Control RaceControls() => Sections(RaceTyreStatusSection(), RacePitSection(), RaceTyreInventorySection(), ModeSection(), EngineSection(), Row(("ERS: ЗАРЯД", "ers_mode", 0), ("ERS: ГИБРИД", "ers_mode", 1), ("ERS: МОЩНОСТЬ", "ers_mode", 2)), Row(("ОТМЕНИТЬ ПИТ", "cancel_pit", 0), ("РЕМОНТ", "pit_repair", 1)), SpeedSection());
    ...
    private Control RaceTyreInventorySection()
    {
        var box = PlanBox("ДОСТУПНЫЕ КОМПЛЕКТЫ"); box.Width = 780;
        _tyreInventory.Width = 740; _tyreInventory.Height = 220; _tyreInventory.ReadOnly = true; _tyreInventory.AllowUserToAddRows = false; _tyreInventory.AllowUserToDeleteRows = false; _tyreInventory.AllowUserToResizeRows = false; _tyreInventory.RowHeadersVisible = false; _tyreInventory.SelectionMode = DataGridViewSelectionMode.FullRowSelect; _tyreInventory.MultiSelect = false; _tyreInventory.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill; _tyreInventory.BackgroundColor = DashboardForm.UiCanvas; _tyreInventory.GridColor = DashboardForm.UiRaised; _tyreInventory.BorderStyle = BorderStyle.None; _tyreInventory.EnableHeadersVisualStyles = false; _tyreInventory.ColumnHeadersDefaultCellStyle = new DataGridViewCellStyle { BackColor = DashboardForm.UiRaised, ForeColor = DashboardForm.UiMuted, Font = new Font("Bahnschrift", 8f), SelectionBackColor = DashboardForm.UiRaised }; _tyreInventory.DefaultCellStyle = new DataGridViewCellStyle { BackColor = DashboardForm.UiCanvas, ForeColor = DashboardForm.UiText, SelectionBackColor = Color.FromArgb(52, 60, 70), SelectionForeColor = DashboardForm.UiText, Font = new Font("Bahnschrift", 9f) };
        _tyreInventory.Columns.Add("set", "КОМПЛЕКТ"); _tyreInventory.Columns.Add("condition", "СОСТОЯНИЕ"); _tyreInventory.Columns.Add("laps", "КРУГИ"); _tyreInventory.Columns.Add("speed", "СКОРОСТЬ"); _tyreInventory.Columns.Add("durability", "ДОЛГОВЕЧНОСТЬ"); _tyreInventory.Columns.Add("temperature", "ТЕМПЕРАТУРА");
        box.Controls.Add(_tyreInventory); return box;
    }
    ...
    private void ApplyEngineeringTelemetry(JsonElement root, JsonElement vehicle)
    {
        ...
        PopulateTyreInventory(ReadTyres(vehicle));
    }
    ...
    private void PopulateTyreInventory(List<TyreChoice> tyres) { int firstVisible = _tyreInventory.FirstDisplayedScrollingRowIndex; _tyreInventory.Rows.Clear(); foreach (TyreChoice tyre in tyres) { string marker = tyre.Current ? "● " : ""; string laps = tyre.MinLaps == tyre.MaxLaps ? tyre.MaxLaps.ToString() : $"{tyre.MinLaps}–{tyre.MaxLaps}"; int row = _tyreInventory.Rows.Add($"{marker}{tyre.Name} #{tyre.Index + 1}", $"{tyre.Condition:0}%", laps, $"{tyre.Speed:0}%", $"{tyre.Durability:0}%", tyre.Current ? $"{tyre.Temperature:0}%" : "—"); if (tyre.Current) _tyreInventory.Rows[row].DefaultCellStyle.ForeColor = DashboardForm.UiGood; } if (firstVisible >= 0 && _tyreInventory.Rows.Count > 0) _tyreInventory.FirstDisplayedScrollingRowIndex = Math.Min(firstVisible, _tyreInventory.Rows.Count - 1); }
    ...
    private static List<TyreChoice> ReadTyres(JsonElement vehicle) { var result = new List<TyreChoice>(); if (!vehicle.TryGetProperty("tyres", out var values)) return result; foreach (JsonElement value in values.EnumerateArray()) result.Add(new(value.GetProperty("option").GetInt32(), value.GetProperty("index").GetInt32(), value.GetProperty("name").GetString() ?? "", ReadOptionalDouble(value, "condition"), ReadOptionalDouble(value, "temperature"), ReadOptionalInt(value, "minLaps", 0), ReadOptionalInt(value, "maxLaps", 0), ReadOptionalDouble(value, "speed"), ReadOptionalDouble(value, "durability"), ReadOptionalText(value, "tread"), ReadOptionalBool(value, "current"))); return result; }
